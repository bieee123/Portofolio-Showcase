var LEVELS = [
            ...
            function updateState(state, keys) {
                let player = state.player.update(keys, state, time);
                let newState = new State(state.level, player, state.status);

                if (state.level.touches(player.pos, player.size, "lava")) {
                    newState.status = "lost";
                    newState.finishDelay = 1;
                }
